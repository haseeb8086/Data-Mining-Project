  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "mid2_exam",
              ...
              fluidRow(
                verbatimTextOutput("prediction_output_mid2")
              )
      ),
      ...
      tabItem(tabName = "final_exam",
              ...
              fluidRow(
                verbatimTextOutput("prediction_output_final")
              )
      )
    )
  )
...
server <- function(input, output) {
  ...
  train_decision_tree <- reactive({
    preprocessed_data <- preprocess_data()
    
    # Train the Decision Tree model using rpart
    decision_tree_mid2 <- rpart(Grade ~ ., data = preprocessed_data$mid2)
    decision_tree_final <- rpart(Grade ~ ., data = preprocessed_data$final)
    
    trained_models <- list(mid2 = decision_tree_mid2, final = decision_tree_final)
  ...
  predict_mid2 <- function() {
    trained_models <- train_decision_tree()
